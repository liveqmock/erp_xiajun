...
//    }
	...
	/**
	 * TODO 是否要抽出去放到flex专门的目录下？
	 * 
	 * @param list
	 * @return
	 */
	public static List<Long> getLongList(List<Long> list) {
		if(list instanceof ArrayCollection) {
    		List<Long> newIdList = new ArrayList<Long>();
    		Object[] objs = ((ArrayCollection)list).getSource();
    		for(int i=0;i<objs.length;i++) {
    			newIdList.add(Long.valueOf(objs[i].toString()));
    		}
    		return newIdList;
		} else {
    		List<Long> newIdList = new ArrayList<Long>();
    ...
    /**
     * 字符串str添加某个没有包含字符串subStr
     * 1.判断str字符串是否包含了subStr
     * 2.如果没有包含，进行连接
     */
    ...
    public static String clearUnknownChar(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if ((c1 < str.charAt(i) && str.charAt(i) < c2) || '\n' == str.charAt(i) || '\t' == str.charAt(i) || '\r' == str.charAt(i) || '\b' == str.charAt(i) || '\f' == str.charAt(i)) {
