...
public class AutoYouzanOrderTask {
    ...
    public void run() {
		...
		for (JdShopOauthDO shopOauth : shopOauthDOList) {
			...
			logger.info("youzan get orders begin time: " + DateUtil.convertDate2Str(beginTime,DateUtil.formateStr19));
			logger.info("youzan get orders end time: " + DateUtil.convertDate2Str(endTime,DateUtil.formateStr19));
			...
			try {
				...
				success = false;
				logger.error("youzan get orders error:",e);
			} catch (Exception e) {
				success = false;
				logger.error("youzan get orders error, shopCode: " + shopOauth.getShopCode(), e);
			}
			if(success){
		...
		logger.info("youzan get orders end, use time:" + (endTime - startTime) +" ms. shopCount: " + shopCount);
    }
	...
	public void sendRequestOrder2erp() {
		...
		for (JdShopOauthDO shopOauth : shopOauthDOList) {
			...
			for(JdOrderDO requestJdOrder : requestJdOrderIList){
				...
				try {
					...
					logger.error("youzan send request Orders error:",e);
				} catch (Exception e) {
					success = false;
					errorMsg += e.getMessage();
					logger.error("youzan send request Orders error, shopCode: " + shopOauth.getShopCode(), e);
				}
		...
		Long endTime = System.currentTimeMillis();
		logger.info("youzan send request Orders end, use time:" + (endTime - startTime) +" ms. shopCount: " + shopCount);
	}
	...
	public void sendFailureOrder2erp() {
		...
		for (JdShopOauthDO shopOauth : shopOauthDOList) {
			...
			for(JdOrderDO failureJdOrder : failureJdOrderList){
				...
				try {
					...
					success = false;
					errorMsg += e.getErrorMsg();
					logger.error("youzan send failure Orders error:",e);
				} catch (Exception e) {
					success = false;
					errorMsg += e.getMessage();
					logger.error("youzan send failure Orders error, shopCode: " + shopOauth.getShopCode(), e);
				}
				if(success){
		...
		Long endTime = System.currentTimeMillis();
		logger.info("youzan send failure orders end, use time:" + (endTime - startTime) +" ms. shopCount: " + shopCount);
	}
}
