...
public class JdGetOrdersTask {
	...
	public void run() {
		...
		for (JdShopOauthDO shopOauth : jdShopOauthDOS) {
			...
			try {
				List<JdOrderDO> jdOrderDOS = JdShopFactory.getChannel(shopOauth).getOrders(DateUtil.convertDate2Str(beginTime,DateUtil.formateStr19), DateUtil.convertDate2Str(endTime,DateUtil.formateStr19));

				if(!EasyUtil.isListEmpty(jdOrderDOS)){
					jdOrderService.saveOrdes4Task(jdOrderDOS);
				}
