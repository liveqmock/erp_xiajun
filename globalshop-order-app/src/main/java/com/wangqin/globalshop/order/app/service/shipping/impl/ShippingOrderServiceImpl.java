...
public class ShippingOrderServiceImpl implements IShippingOrderService {
    ...
    public MultiDeliveryFormDTO queryByOrderId(String orderId) {
        ...
        for (MallSubOrderDO mallSubOrder : mallSubOrderList) {
            if (mallSubOrder.getStockStatus() == null
                || (mallSubOrder.getStockStatus() != StockUpStatus.STOCKUP.getCode()
                    && mallSubOrder.getStockStatus() != StockUpStatus.PREPARE.getCode())) {
                throw new ErpCommonException("商品备货状态不对，子订单号：" + mallSubOrder.getId());
            }
            if (StringUtils.isBlank(mallSubOrder.getReceiver()) || StringUtils.isBlank(mallSubOrder.getTelephone())
                || StringUtils.isBlank(mallSubOrder.getReceiverState())
                || StringUtils.isBlank(mallSubOrder.getReceiverCity())
                || StringUtils.isBlank(mallSubOrder.getReceiverDistrict())) {
            ...
            } else if (!receiver.equals(mallSubOrder.getReceiver()) || !telephone.equals(mallSubOrder.getTelephone())
                       || !addressDetail.equals(mallSubOrder.getReceiverAddress())) {
            ...
            for (int i = 0; i < selErpOrderList.size(); i++) {
                ...
                if (selErpOrder.getWarehouseNo() != null
                    && selErpOrder.getWarehouseNo() != mallSubOrder.getWarehouseNo()) {
                    continue;
                }
                // 在同一仓库的或者未备货的
    ...
    public Map<String, Set<Long>> batchDelivery(ShippingOrderDO shippingOrder) throws InventoryException {
        ...
        /*
         * if(shippingOrder.getIsBatch()==0 && erpOrderIdList.size()>1) { throw new
         * ErpCommonException("此操作仅为一个子订单发货，而您选中了多个子订单！"); }
         */
        ...
        for (MallSubOrderDO erpOrder : erpOrderList) {
            ...
            shippingOrder.setShippingNo("PKG" + DateUtil.formatDate(nowDate, DateUtil.DATE_PARTEN_YYMMDDHHMMSS)
                                        + sequenceUtilMapperExt.gainPKGSequence());
        ...
        try {
            ...
            ChannelFactory.getChannel(ShiroUtil.getShiroUser().getCompanyNo(),
                                      ChannelType.getChannelType(outerOrder.getPayType())).syncLogisticsOnlineConfirm(erpOrderList,
                                                                                                                      shippingOrder);
        } catch (Exception e) {
