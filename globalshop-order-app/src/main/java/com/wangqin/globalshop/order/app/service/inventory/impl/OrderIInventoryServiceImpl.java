...
//        Map<Long,Integer> warehouseSeqMap = inventoryOnWarehouseService.getWarehouseSeqMap();
        ...
        for (MallSubOrderDO erpOrder : erpOrders) {
            ...
            if (CollectionUtils.isNotEmpty(sumInventorys)) {
                for (InventoryOnWareHouseDO inventory : sumInventorys) {
                    Long totalAvailableInv = inventory.getInventory() - inventory.getLockedInv() + inventory.getTransInv() - inventory.getLockedTransInv();
                    if (totalAvailableInv > 0) {
                        WarehouseCollector whc = new WarehouseCollector();
                        whc.setOrderNo(erpOrder.getOrderNo());
                        whc.setWarehouseNo(inventory.getInventoryOnWarehouseNo());
                        whc.setQuantity(erpOrder.getQuantity());
                        whc.setErpOrder(erpOrder);
                        initBooked(inventory, whc);
                        table.put(inventory.getInventoryOnWarehouseNo(), whc.getOrderNo(), whc);
                    }
                }
            }
        }
            ...
            for (String wareId : scoreMap.keySet()) {
                if (maxD == null) {
                    ...
                    if (maxD < scoreMap.get(wareId)) {
                        maxD = scoreMap.get(wareId);
                        maxWareId = wareId;
                    } else if(maxD.equals(scoreMap.get(wareId))
//                            && warehouseSeqMap.get(wareId) > warehouseSeqMap.get(maxWareId)
                            ) {
                        maxD = scoreMap.get(wareId);
                        maxWareId = wareId;
                    }
    ...
    public void lockedInventroy(WarehouseCollector warehouseCollector) throws InventoryException {
        ...
        // 悲观锁 锁定库存记录，防止并发
        InventoryDO inventory = inventoryMapper.getInventoryBySkuId(itemId, skuId);
        Long availableInv = inventory.getInv() - inventory.getLockedInv();// 可售库存
        Long availableTransInv = inventory.getTransInv() - inventory.getLockedTransInv();// 可用在途库存
        //校验库存是否足够
        if(availableInv<warehouseCollector.getBooked()||availableTransInv<warehouseCollector.getTransBooked()){
            throw new InventoryException(String.format(
                    ...
                    inventory.getTransInv() - inventory.getLockedTransInv(), availableTransInv));
        }
        ...
        mallsubOrderService.update(erpOrder);//更新状态
    }
    ...
    private void insertInventoryLockedRecord(MallSubOrderDO erpOrder, WarehouseCollector warehouseCollector, Long availableInv)
       ...
       String warehouseId = warehouseCollector.getWarehouseNo();
        ...
        for (InventoryOnWareHouseDO inventoryArea : areas) {
            Long available = inventoryArea.getAvailableInv();
            if (available == 0) {
                continue;
            }
            Long bookedArea = 0L;
            ...
            inventoryRecordService
                    .insert(buildLockedRecord(erpOrder, bookedArea, inventoryArea, skuId, InventoryRecord.InventoryType.INVENTORY));
            // 预定量为0直接推出循环
            if (booked == 0) {
                break;
            }
