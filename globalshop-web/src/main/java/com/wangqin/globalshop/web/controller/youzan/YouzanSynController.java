package com.wangqin.globalshop.web.controller.youzan;
		...
		if (StringUtils.isNotEmpty(itemIds)) {
			...
			if (CollectionUtils.isNotEmpty(idList)) {
				...
				for (ItemDO item : items) {
					...
					for (ChannelAccountDO channelAccount : channelAccountList){
						try {
							ChannelFactory.getChannel(channelAccount).syncListingItem(item);
						} catch (Exception e) {
							e.printStackTrace();
			...
			if (CollectionUtils.isNotEmpty(idList)) {
				...
				for (ItemDO item : items) {
					...
					for (ChannelAccountDO channelAccount : channelAccountList){
						try {
							// 有赞
							ChannelFactory.getChannel(channelAccount).syncDelistingItem(item);
						} catch (Exception e) {
