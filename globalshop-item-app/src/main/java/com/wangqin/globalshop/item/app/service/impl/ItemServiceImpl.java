...
//		}
	...
	public void updateItem(ItemDO item) {
		...
		//获取数据库里面的所有sku
		/*Item itemDb = queryItem(item.getId());
		List<ItemSkuDO> itemSkuListDb = itemDb.getItemSkus();
		...
		if(!itemSkuListUpdate.isEmpty()){
			...
			for(ItemSkuDO itemSku:itemSkuListUpdate){
				InventoryDO inventory = inventoryService.queryInventoryBySkuId(itemSku.getItemId(), itemSku.getId());
				if(inventory==null){
			...
			if(CollectionUtils.isNotEmpty(newInvList)){
				List<InventoryDO> inventoryList = itemSkuService.initInventory(newInvList);
				inventoryService.insertBatch(inventoryList);
			}
		...
		if(!itemSkuListNew.isEmpty()){
			...
			List<InventoryDO> inventoryList = itemSkuService.initInventory(itemSkuListNew);
			inventoryService.insertBatch(inventoryList);
		}
		if(!allSkus.isEmpty()){
