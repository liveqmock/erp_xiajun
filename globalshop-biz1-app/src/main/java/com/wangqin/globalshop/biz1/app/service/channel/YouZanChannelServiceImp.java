...
public class YouZanChannelServiceImp extends AbstractChannelService implements IChannelService {
	...
	void syncOrder(TradeDetailV2 TradeDetail) {
		...
		p.setTargetNo(TradeDetail.getTid());
		...
		outerOrder.setReceiver(TradeDetail.getReceiverName()); // 收货人
		outerOrder.setReceiverState(TradeDetail.getReceiverState()); // 省
		outerOrder.setReceiverCity(TradeDetail.getReceiverCity()); // 市
		outerOrder.setReceiverDistrict(TradeDetail.getReceiverDistrict()); // 区
		outerOrder.setAddressDetail(TradeDetail.getReceiverAddress()); // 详细地址
		outerOrder.setTelephone(TradeDetail.getReceiverMobile()); // 联系电话
		outerOrder.setPostcode(TradeDetail.getReceiverZip()); // 邮编
		outerOrder.setRemark(TradeDetail.getBuyerMessage()); // 买家购买附言
		outerOrder.setTargetNo(TradeDetail.getTid()); // 有赞平台交易编号
